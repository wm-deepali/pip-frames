    ...
    <form id="attribute-value-form" enctype="multipart/form-data">
      ...
      <div class="modal-body">
        ...
        <div class="form-group" id="value-wrapper">
          ...
          <?php if($inputType === 'select_image' && $requireBothImages): ?>
            <label>Portrait Image (optional)</label>
            <input type="file" name="image_portrait" class="form-control-file mb-2">
            <?php if(!empty($attributeValue->image_portrait_path)): ?>
              <div class="mt-2">
                <strong>Existing Portrait Image:</strong><br>
                <img src="<?php echo e(asset('storage/' . $attributeValue->image_portrait_path)); ?>" width="80" alt="Portrait Image">
              </div>
            <?php endif; ?>

            <label>Landscape Image (optional)</label>
            <input type="file" name="image_landscape" class="form-control-file">
            <?php if(!empty($attributeValue->image_landscape_path)): ?>
              <div class="mt-2">
                <strong>Existing Landscape Image:</strong><br>
                <img src="<?php echo e(asset('storage/' . $attributeValue->image_landscape_path)); ?>" width="80" alt="Landscape Image">
              </div>
            <?php endif; ?>

          <?php elseif(in_array($inputType, ['select_image', 'select_icon'])): ?>
        ...
        <?php if($attribute->required_file_uploads): ?>
          <div class="form-group" id="required-file-uploads-wrapper">
            ...
            <input type="number" name="required_file_uploads" id="required_file_uploads" class="form-control" min="0"
              value="<?php echo e(old('required_file_uploads', $attributeValue->required_file_uploads ?? '')); ?>"
        ...
        <?php if($inputType === 'select_colour'): ?>
          <div class="form-group" id="colour-fields-wrapper">
            ...
            <input type="text" name="colour_code" class="form-control colour-code-input"
              value="<?php echo e($attributeValue->colour_code ?? ''); ?>" placeholder="#FF0000">
            ...
            <input type="color" name="colour_picker" class="form-control form-control-color colour-picker-input"
              value="<?php echo e($attributeValue->colour_code ?? '#ffffff'); ?>" style="width: 60px; height: 38px; padding: 2px;">
...
<script>
  $(document).ready(function () {
    ...
    const requiredFileUploads = <?php echo json_encode($attribute->required_file_uploads, 15, 512) ?>;
    const parentImagesData = <?php echo json_encode($parentImagesData, 15, 512) ?>;
    const attributeConfig = <?php echo json_encode($attributeConfigs[$attribute->id], 15, 512) ?>;
