                        ...
                        <div class="col-12">
                            <div class="breadcrumb-wrapper">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="{{ route('admin.images.index') }}">Image
                                            Settings</a></li>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form id="image-settings-form" method="POST" enctype="multipart/form-data"
                            action="{{ route('admin.images.update', $condition->id) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="form-group">
                                ...
                                <select name="subcategory_id" class="form-control" id="subcategory-select">
                                    ...
                                    @foreach($subcategories as $subcategory)
                                        <option value="{{ $subcategory->id }}" {{ $condition->subcategory_id == $subcategory->id ? 'selected' : '' }}>
                                            {{ $subcategory->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div id="conditions-container">
                                @foreach($condition->dependencies->groupBy('condition_id') as $condIndex => $deps)
                                    <div class="condition-block border rounded p-1 mb-1">
                                        <input type="hidden" name="conditions[{{ $loop->index }}][id]"
                                            value="{{ $condition->id }}">

                                        {{-- Dependencies --}}
                                        <div class="dependencies-container">
                                            @foreach($deps as $depIndex => $dep)
                                                <div class="dependency-row row mb-1">
                                                    <div class="form-group col-md-5">
                                                        <label><strong>Attribute</strong></label>
                                                        <select
                                                            name="conditions[{{ $loop->parent->index }}][dependencies][{{ $loop->index }}][attribute_id]"
                                                            class="form-control dependency-attribute-select">
                                                            <option value="">-- Select Attribute --</option>
                                                            @foreach($attributes as $attr)
                                                                <option value="{{ $attr['id']}}" {{ $dep->attribute_id == $attr['id'] ? 'selected' : '' }}>
                                                                    {{ $attr['name'] }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <div class="form-group col-md-5">
                                                        ...
                                                        <select
                                                            name="conditions[{{ $loop->parent->index }}][dependencies][{{ $loop->index }}][value_id]"
                                                            class="form-control dependency-value-select">
                                                            <option value="">-- Select Value --</option>

                                                            @if(isset($attributeValueMap[$dep->attribute_id]))
                                                                @foreach($attributeValueMap[$dep->attribute_id] as $val)
                                                                    <option value="{{ $val['id'] }}" {{ $dep->value_id == $val['id'] ? 'selected' : '' }}>{{ $val['value'] }}
                                                                    </option>
                                                                @endforeach
                                                            @endif
                                        ...
                                        <div class="row mb-2">
                                            <div class="form-group col-md-6">
                                                ...
                                                <select name="conditions[{{ $loop->index }}][affected_attribute_id]"
                                                    class="form-control affected-attribute-select">
                                                    <option value="">-- Select Attribute --</option>
                                                    @foreach($attributes as $attr)
                                                        <option value="{{ $attr['id'] }}" {{ $condition->affected_attribute_id == $attr['id'] ? 'selected' : '' }}>
                                                            {{ $attr['name'] }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="form-group col-md-6">
                                                ...
                                                <div class="scrollable-checkbox-box affected-values-checkboxes">
                                                    @if(isset($attributeValueMap[$condition->affected_attribute_id]))
                                                        @foreach($attributeValueMap[$condition->affected_attribute_id] as $val)
                                                            @php
                                                            ...
                                                            <div class="form-check d-flex align-items-center mb-1">
                                                                <input type="checkbox"
                                                                    name="conditions[{{ $loop->parent->index }}][affected_value_ids][]"
                                                                    value="{{ $val['id'] }}"
                                                                    id="value-{{ $loop->parent->index }}-{{ $val['id'] }}" {{ $affected ? 'checked' : '' }}>
                                                                <label class="form-check-label mr-2"
                                                                    for="value-{{ $loop->parent->index }}-{{ $val['id'] }}">
                                                                    {{ $val['value'] }}
                                                                </label>

                                                                <input type="file"
                                                                    name="conditions[{{ $loop->parent->index }}][value_images][{{ $val['id'] }}]"
                                                                    class="form-control-file ml-2 d-none browse-input" accept="image/*">

                                                                <select class="form-control ml-2" style="max-width: 120px;"
                                                                    name="conditions[{{ $loop->parent->index }}][orientation][{{ $val['id'] }}]">
                                                                    <option value="">Orientation</option>
                                                                    <option value="landscape" {{ (isset($affected) && $affected->orientation == 'landscape') ? 'selected' : '' }}>
                                                                        Landscape</option>
                                                                    <option value="portrait" {{ (isset($affected) && $affected->orientation == 'portrait') ? 'selected' : '' }}>
                                                                        Portrait</option>
                                                                </select>

                                                                @if($affected && !empty($affected->image))
                                                                    <img src="{{ asset('storage/' . $affected->image) }}" alt="img"
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.success) {
                                ...
                                setTimeout(() => window.location.href = "{{ route('admin.images.index') }}", 800);
                            } else {
